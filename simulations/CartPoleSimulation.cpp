...
#define POLE_MASS 1.0f
#define TOTAL_MASS (CART_MASS + POLE_MASS)
#define POLE_LENGTH 75.0f // actually half the pole's length
#define FORCE_MAG 100.0f
...
void CartPoleSimulation::eval(int tid, int block, float **actions, float *gamestate)
{
    ...
    float force = (actions[0][0] > 0) ? FORCE_MAG : -FORCE_MAG;
    if(abs(actions[0][0]) < FORCE_MAG){
        force = actions[0][0];
    }
    gamestate[6] = force; //Record action taken this iteration
...
{
    // The simulation ends if the pole is more than 12 degrees from vertical or the cart moves more than 12.4 units from the center
    if (abs(gamestate[2]) > 12 * M_PI / 180 || abs(gamestate[0]) > 12.4) {
...
Eigen::MatrixXd CartPoleSimulation::getState(float & action, float & reward, float *gamestate)
{
    action = gamestate[6];
    reward = gamestate[5];
    MatrixXd state(1, 4);
